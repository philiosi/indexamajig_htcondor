...
usage() {
        err_msg "Usage: $0 -g pal.geom -i mosflm -j 36 -f r0081c00.lst or all -o SASE.stream -p mycell.pdb -e "--int-radius=3,4,5 --threshold=600 --min-srn=4 --min-gradient=100000" "
}
err_msg() { echo "$@" ; } >&2
err_msg_f() { err_msg "-f option requires input file list";}
err_msg_g() { err_msg "-g option requires geometry file (ex: pal.geom)";}
...
# move echo "indexmajig ..." command from if-then statements (l:95) by shna 2022-07-01
#echo "indexamajig" "-g" $g "--indexing="$i "-j "$j "-f "$f "-o "$i_$o $e "-p "$p  
...
if [ $f == "all" ]; then
	...
	do
		...
		echo "indexamajig" "-g" $g "--indexing="$i "-j "$j "-f "$runnum_lst "-o "$runnum_$i_$streamname_$o $e "-p "$p
		...
		arguments = ${PROCDIR} ${g} ${i} ${j} ${runnum_lst} ${runnum} ${o} ${p} ${e}
		queue
		EOF
	...
	echo "indexamajig" "-g" $g "--indexing="$i "-j "$j "-f "$f "-o "$runnum_$i_$streamname_$o $e "-p "$p
	...
	request_memory = $MEM GB
	executable = 3_exec_indexing.sh
	arguments = ${PROCDIR} ${g} ${i} ${j} ${f} ${runnum} ${o} ${p} ${e} 
	queue
	EOF
